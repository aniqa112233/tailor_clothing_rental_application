// // lib/screens/order/order_create_screen.dart
// import 'dart:convert';
...
//     final tailorsRes = await supabase.from('tailors').select('*');
//     final servicesRes = await supabase.from('services').select('*');
//     if (tailorsRes.error != null || servicesRes.error != null) {
//       ScaffoldMessenger.of(context).showSnackBar(
//         SnackBar(content: Text('Error fetching tailors/services')),
//       );
//     } else {
//       _tailors = (tailorsRes as List).map((e) => e as Map<String, dynamic>).toList();
//       _services = (servicesRes as List).map((e) => e as Map<String, dynamic>).toList();
//     }
...
//   // If user types JSON measurements into text area, we parse it.
...
//     final dateText = _deliveryDate == null ? 'Select delivery date' : DateFormat.yMMMd().format(_deliveryDate!);
//     return Scaffold(
...
//                     items: _services
//                         .where((s) => s['type'] == _orderType)
//                         .map((s) => DropdownMenuItem(
//                               value: s['id'] as String,
//                               child: Text(s['title'] ?? s['name'] ?? 'Service'),
//                             ))
//                         .toList(),
...
//                               child: Text(t['display_name'] ?? t['shop_name'] ?? 'Tailor'),
//                             ))
//                         .toList(),
...
//                       helperText: '{"chest": 36, "waist": 28}',
//                       border: OutlineInputBorder(),
//                     ),
//                     onChanged: (t) {
//                       _onMeasurementsChanged(t);
//                     },
//                     validator: (v) {
//                       if ((v ?? '').isEmpty) return null; // optional
//                       try {
//                         final parsed = jsonDecode(v!);
//                         if (parsed is! Map) return 'Invalid JSON object';
//                       } catch (e) {
//                         return 'Invalid JSON';
//                       }
//                       return null;
...
//   // ✅ Fixed version — uses try/catch instead of .error
//   Future<void> _fetchTailorsAndServices() async {
//     try {
//       setState(() => _loading = true);
...
//       // ✅ Safely cast with .toString() to fix empty dropdowns
//       _tailors = (tailorsData as List)
//           .map((e) => e as Map<String, dynamic>)
//           .toList();
//       _services = (servicesData as List)
//           .map((e) => e as Map<String, dynamic>)
//           .toList();
//     } catch (e) {
//       ScaffoldMessenger.of(context).showSnackBar(
...
//                           .where((s) => s['type'] == _orderType)
//                           .map((s) => DropdownMenuItem(
//                                 value: s['id'].toString(), // ✅ fix: convert UUID safely
//                                 child: Text(
//                                   s['title']?.toString() ??
//                                       s['name']?.toString() ??
//                                       'Unnamed Service',
...
//                       onChanged: (v) => setState(() => _selectedServiceId = v),
//                       validator: (v) =>
//                           v == null ? 'Please select service' : null,
//                     ),
...
//                       value: _selectedTailorId,
//                       decoration: const InputDecoration(
//                           labelText: 'Assign tailor (optional)'),
//                       items: _tailors
//                           .map((t) => DropdownMenuItem(
//                                 value: t['id'].toString(), // ✅ fix
//                                 child: Text(
...
//               final insert = await supabase.from('services').insert({
...
//                                 .map((s) => DropdownMenuItem(
//                                       value: s['id'].toString(),
//                                       child: Text(s['title']?.toString() ?? 'Unnamed Service'),
//                                     ))
//                                 .toList(),
//                             onChanged: (v) => setState(() => _selectedServiceId = v),
//                             validator: (v) => v == null ? 'Please select service' : null,
...
//   // ✅ Submit order + auto insert extra fields
//   Future<void> _submit() async {
//     if (!_formKey.currentState!.validate()) return;
//     if (_selectedServiceId == null) {
//       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Select a service')));
...
//   // 🆕 Add Service (safe for both 'title' or 'name')
//   Future<void> _addNewService() async {
//     final titleController = TextEditingController();
//     final priceController = TextEditingController();
...
//               if (titleController.text.isEmpty) return;
//               try {
//                 // ✅ Check which column exists: 'title' or 'name'
//                 final insertData = {
//                   'type': _orderType,
//                   'price': double.tryParse(priceController.text) ?? 0,
//                   'created_at': DateTime.now().toIso8601String(),
