...
class CatalogService {
  ...
  // ✅ FIXED: Fetch all catalog items (official + custom)
  Future<List<ServiceModel>> fetchAllCatalogItems(String category) async {
    // 1️⃣ Official services by category
    final serviceList = await fetchServices(category);

    // 2️⃣ Custom designs → only pending + approved (skip rejected)
    final customResponse = await supabase
        .from('custom_designs')
        .select()
        .inFilter('status', ['pending', 'approved'])
        .order('created_at', ascending: false);

    // 3️⃣ Map custom designs to ServiceModel-like format
  ...
  // ✅ Upload fabric image & save fabric record
  ...
  Future<void> saveFabric(String imageUrl, String name, String type,
      String color, double price, int stock) async {
